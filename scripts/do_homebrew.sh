...
#
# Usage:
#   ./scripts/do_homebrew.sh                 # hash the published DMG and update the cask
#   ./scripts/do_homebrew.sh --sha256 <hash> # trust this hash instead of downloading
#   ./scripts/do_homebrew.sh --dry-run       # print the cask, change nothing
#
...
if [ -z "$SHA" ]; then
    # Hash what users will really download, not what happens to sit in dist/. If a rebuild
